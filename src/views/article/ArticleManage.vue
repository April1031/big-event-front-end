...
import { ArticleCategoryListService, 
                ...
                ArticleDeleteService} from '@/api/article.js';
...
const drawerTitle = ref('添加文章') // 抽屉标题
...
const submitArticle = async (clickState) => {
  ...
  ElMessage.success(result.message || (isEdit.value ? '修改成功' : '添加成功'));
  visibleDrawer.value = false;
  articleList();
};
